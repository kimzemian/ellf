...
def eval(model, loader, split_name, scaler):  # TODO: why is threshold 1 and 5?
    all_preds = []
    all_labels = []
    model.eval()
...
def main(args):
    ...
    train_features = get_train_dataset_features(train_dataset, transform=np.log1p)
    scaler = TorchStandardScaler()
    scaler.fit(train_features)
    print(scaler.mean, scaler.std)  # tensor(3.3463) tensor(2.9714)
    ...
    # Training loop
    wandb.init(project="sweep3", name=exp_tag, config=vars(args))
    step = 0
