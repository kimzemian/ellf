    ...
    for opt in "${OPTS[@]}"; do
      for warmup in "${WARMUPS[@]}"; do
        for sched in "${SCHEDULERS[@]}"; do
          ...
          echo "Running: lr=$lr weight_decay=$wd optimizer=$opt warmup_epochs=$warmup scheduler=$sched"
          python $SCRIPT --lr $lr --weight_decay $wd --optimizer $opt --warmup_epoch $warmup --scheduler $sched --epochs 10
        done
      done
    done
  done
